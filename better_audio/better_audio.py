    ...
    async def metadata_loop(self):
        while True:
            for sid in self.players:               
                if self.voice_clients[sid] is not None:
                    if self.players[sid] is not None:
                        while self.db[sid]["stream"]:
                            ...
                            async with aiohttp.get(url, headers={'Icy-MetaData': "1"}) as r:
                                ...
                                if "Icy-Metaint" in urls:
                                    ...
                                    if track:
                                        ...
                                        print(title)
                                        if self.players[sid] is not None:
                                            if self.playing[sid]["title"] != title and self.playing[sid]["author"] != author:
                                                print("whaa")
                                                print(title)
                                                self.playing[sid]["title"] = title
                                                self.playing[sid]["author"] = author
            await asyncio.sleep(1)
